...
RUN pip install --no-cache-dir -r requirements-web.txt -r requirements-dev.txt
...
EXPOSE 22
RUN pip install --no-cache-dir -r requirements-dev.txt -r requirements-worker.txt
ENTRYPOINT ["sh", "-c", "exec rq worker --url $REDIS_URL $LORELAI_RQ_QUEUES"]
