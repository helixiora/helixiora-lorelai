...
# this file creates a class to process google drive documents using 
# the google drive api, chunk them using langchain and then index them in pinecone
class GoogleDriveProcessor:
    # create or select the pinecone index
    ...
    def __init__(self, pinecone_api_key, pinecone_environment, pinecone_index_name, openai_api_key):
        ...
        self.pinecone = PineconeVectorStore(pinecone_api_key=self.pinecone_api_key, index_name=self.pinecone_index_name, embedding=self.embeddings)

    def processDrive(self):
        ...
        db = self.pinecone.from_documents(documents, self.embeddings, index_name=self.pinecone_index_name)
        
        # vectorized_chunks = [(str(i), self.vectorizer(chunk), {}) for i, chunk in enumerate(chunks)]
